...
describes what users get. Pick a format with `-o`, or let nasacmr choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
nasacmr <command> -o table   # aligned columns for reading
nasacmr <command> -o jsonl   # one JSON object per line, for piping
nasacmr <command> -o json    # a single JSON array
nasacmr <command> -o csv     # spreadsheet friendly
nasacmr <command> -o tsv     # tab-separated
nasacmr <command> -o url     # just the URL column
nasacmr <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
nasacmr <command> --fields id,title,url
```
...
```bash
nasacmr <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
nasacmr <command>            # a table, because this is a terminal
nasacmr <command> | wc -l    # JSONL, because this is a pipe
```
